...
void Module(double slantRange, double azimuth, double xa, double ya, double za) //реализация модуля рсбн
{
    double xb, yb, zb, deltax, deltay, pr, pt, gt, ar, sigma;
    ...
    zb = sqrt(pow(slantRange, 2) - pow((xa - xb), 2));
    
    cout << "Координаты самолета: X = " << xb <<", Y = "<< yb <<", Z = "<<zb << endl <<endl;
...
double* beaconSwapped() //функция для смены радиомаяка
{
    ...
    beacxy[2] = static_cast <double> (rand()) / (static_cast <double> (RAND_MAX / k));
    return beacxy;
}
void Beacon(int slantRange, int azimuth) //реализация радиомаяка
{
    ...
    cout << "Координаты радиомаяка: X = " << beacxy[0]<< ", Y = "<< beacxy[1] << ", Z = " << beacxy[2] << endl;
    Module(slantRange, azimuth, beacxy[0], beacxy[1], beacxy[2]); //передача данных модулю рсбн
}
